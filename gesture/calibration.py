    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Space:
            ...
            if len(self.polygon_points) >= 3 and not self.polygon_closed:
                ...
                keyword = input("\nEnter a name for this calibration:").strip()
                if keyword == "":
                    keyword = "default"
                ...
                save_dir = os.path.join(os.path.dirname(__file__), "calibrations")
                os.makedirs(save_dir, exist_ok=True)

                full_path = os.path.join(save_dir, filename)

                poly_as_tuples = [(p.x(), p.z()) for p in self.polygon_points]

                with open(full_path, 'w') as f:
                    json.dump(poly_as_tuples, f, indent=2)

                print(f"[SAVE] Calibration saved to {full_path}\n")
